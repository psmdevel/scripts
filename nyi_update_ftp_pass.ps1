...
plink -i \scripts\sources\ts01_privkey.ppk root@store09a "cd /_ftpauth/;cp pureftpd.passwd backups/pureftpd.passwd.$DATESTAMP`_$m_SID;cp pureftpd.pdb backups/pureftpd.pdb.$DATESTAMP`_$m_SID"
$TEST_PURE_BACKUP = plink -i \scripts\sources\ts01_privkey.ppk root@store09a "du -hs /_ftpauth/backups/*.$DATESTAMP`_$m_SID"
plink -i \scripts\sources\ts01_privkey.ppk root@store09b "cd /_ftpauth/;cp pureftpd.passwd backups/pureftpd.passwd.$DATESTAMP`_$m_SID;cp pureftpd.pdb backups/pureftpd.pdb.$DATESTAMP`_$m_SID"
$TEST_PURE_BACKUP_B = plink -i \scripts\sources\ts01_privkey.ppk root@store09b "du -hs /_ftpauth/backups/*.$DATESTAMP`_$m_SID"
        ...
        if (!$NYI)
            {
                ...
                invoke-mysql --site=$m_SID --host=$DBCLUST --user=$DBUSER --pass=$DBPWD --query="update ftpconfig set defaultpassword = '$PASSWORD' where defaultusername = 'site$m_SID' limit 1;"
                if ((invoke-mysql --site=$m_SID --host=$DBCLUST --user=$DBUSER --pass=$DBPWD --query="select defaultpassword from ftpconfig where defaultusername = 'site$m_SID'").defaultpassword -eq $PASSWORD)
                    {
                        Write-Host -ForegroundColor Green '[OK]'
                    }
                        else
                ...
                invoke-mysql --site=$m_SID --host=$DBCLUST --user=$DBUSER --pass=$DBPWD --query="update ftp_params set ftppassword = '$PASSWORD' where ftpuserid = 'site$m_SID' limit 1;"
                if ((invoke-mysql --site=$m_SID --host=$DBCLUST --user=$DBUSER --pass=$DBPWD --query="select ftppassword from ftp_params where ftpuserid = 'site$m_SID'").ftppassword -eq $PASSWORD)
                    {
                        Write-Host -ForegroundColor Green '[OK]'
                    }
                        else
                ...
                invoke-mysql --site=$m_SID --host=$DBCLUST --user=$DBUSER --pass=$DBPWD --query="update ftp_params_era set ftppassword = '$PASSWORD' where ftpuserid = 'site$m_SID' limit 1;"
                if ((invoke-mysql --site=$m_SID --host=$DBCLUST --user=$DBUSER --pass=$DBPWD --query="select ftppassword from ftp_params_era where ftpuserid = 'site$m_SID'").ftppassword -eq $PASSWORD)
                    {
                        Write-Host -ForegroundColor Green '[OK]'
                    }
                        else
