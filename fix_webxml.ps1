...
if ($HELP) {
[PSCustomObject] @{
'Description' = "Checks for mobiledoc/WEB-INF/web.xml existence and replaces it if necessary. "
'--help|-h' = "Display available options"
'--site|-s' = "Specify the site number. Defaults to all sites"
'--force|-f' = "Force copy of web.xml from source"
                }|Format-List; exit
            }
...
if ($m_SID)
    {
     ...
     else
        {
            ...
            $LIST = Invoke-MySQL -s=000 --query="select siteid from sitetab where status = 'active' order by siteid;"
        ...
        if ($TESTWEBXML1 -eq $FALSE -and $TESTWEBXML2 -eq $FALSE -or $FORCE)
            {
                ...
                $CVPatch = Invoke-MySQL --site=$SID --host=$DBCLUSTER --user=$DBUSER --pass=$DBPWD --query="select ecwpatchid from patcheslist where patchdescription = '$CV';"
                ...
                if ($TESTPATCH -eq $FALSE)
                    {Write-Host "web.xml file not found for patch_$PATCH";if ($m_SID){exit}}
