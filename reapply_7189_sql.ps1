...
if ($HELP) {
[PSCustomObject] @{
'Description' = 'Reapplies specific pieces of the 7189 patch'
...
if ($m_SID)
    {
        $site_array += $m_SID
            ...
            {
                foreach ($s in (query_mass_deploy.ps1 -p=7189 --nosummary).siteid|Sort-Object $REVERSE)
                    {
                        $site_array += $s
                    }
            }
...
[xml]$sqlversions = Get-Content M:\scripts\PatchCentral\patches\patch_7189\Tool\runtime\sql\insertupdate.xml
$versions = (($sqlversions).queries.query|where {$_.dbvendor -ne 'mssql'}).version
foreach ($SID in $site_array)
    {
        
        if (-not(Get-Content M:\scripts\PatchCentral\temp\mass_deployment\7189\rerun.txt|Select-String "site$SID completed","site$SID started") -or $NOLOG)
            {
                ...
                if (!$NOLOG)
                    {
                        Write-Output "site$SID started"|Out-File M:\scripts\PatchCentral\temp\mass_deployment\7189\rerun.txt -Append
                    }
                if ($USESCRIPT)
                    {
                        m:\scripts\patchcentral\patch_minor.cmd -p:7189 -f -y -r --skipcopywebapps --site:$SID
                    }
                if ($ECWAPU2)
                    {
                        ...
                        if ((Get-ChildItem -File \\$APP2\site$SID\tomcat7\conf\web.xml).Length -ne 51619)
                            {
                                ...
                                cpi M:\scripts\PatchCentral\patches\patch_7189\Server\conf\web.xml \\$APP1\site$SID\tomcat7\conf\
                                cpi M:\scripts\PatchCentral\patches\patch_7189\Server\conf\web.xml \\$APP2\site$SID\tomcat7\conf\
                ...
                if (!$NOLOG)
                    {
                        Write-Output "site$SID completed"|Out-File M:\scripts\PatchCentral\temp\mass_deployment\7189\rerun.txt -Append
                    }
