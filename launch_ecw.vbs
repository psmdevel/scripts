...
'--create launch_ecw logs folder if necessary 
If Not m_objFS.FolderExists("m:\scripts\logs") Then
	m_objFS.CreateFolder "m:\scripts\logs" 
End If
If Not m_objFS.FolderExists("m:\scripts\logs\launch_ecw") Then
	m_objFS.CreateFolder "m:\scripts\logs\launch_ecw" 
End If
...
If m_boolEnableDebug=true Then
	Set objLog=m_objFS.OpenTextFile("m:\scripts\logs\launch_ecw\" & strLogonName & ".log",FOR_APPEND,True)
End If
...
'--create app datapsm folder if necessary
...
'--finish log entires for this launch event
DebugIt "launching eCW; script will complete without waiting"
If m_objFS.FolderExists("C:\Program Files\eClinicalWorks") Then
	strCmd="""c:\program files\eclinicalworks\eclinicalworks.exe"""
	DebugIt "executing: " & strCmd
	m_objShell.Run strCmd,0,False	
End If
DebugIt "script complete"
